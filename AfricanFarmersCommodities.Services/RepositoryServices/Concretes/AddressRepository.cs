...
namespace SimbaToursEastAfrica.Services.RepositoryServices.Concretes
{
    public class AddressRepository : AbstractRepository<Address>
    {
        public override bool Delete(Address toDelete)
        {
            try
            {
                toDelete = AfricanFarmerCommoditiesDBContext.Addresses.SingleOrDefault(p => p.AddressId == toDelete.AddressId);
        ...
        {
            return AfricanFarmerCommoditiesDBContext.Addresses.SingleOrDefault(p => p.AddressId == id);
        }
